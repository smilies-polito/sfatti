...
class Optimizer(Trainer, NetBuilder):
	...
	def optimize(self, dataloader):
		"""
		Sweep fp_dec ∈ optim_config["fp_dec"], weights_bw ∈ optim_config["weights_bw"],
		neurons_bw ∈ optim_config["neurons_bw"]. Evaluate each quantized model on 'dataloader'
		...
		"""
		# Prepare table headers (for logging) and initialize empty results list
		headers = ["Fixed-point decimals", "Neurons' bitwidth", "Weights bitwidth", "Loss", "Accuracy"]
		table = []
		results = []

		# Triple‐nested loop over fp_dec, weights_bw, neurons_bw
		for fp_dec in self.optim_config["fp_dec"]:
			for w_bw in self.optim_config["weights_bw"]:
				for neuron_bw in self.optim_config["neurons_bw"]:
					# Build a fresh QuantSNN with this (w_bw, neuron_bw, fp_dec)
					self.build_quant_snn(w_bw, neuron_bw, fp_dec)

					# Evaluate on the provided dataloader (returns loss, acc in [0,1])
					loss, acc = self.evaluate(dataloader)
					acc_pct = float(acc * 100.0)

					# Log the single-run result
					log_message = (
						f"\nLoss: {loss:.2f}\n"
						f"Acc: {acc_pct:.2f}%\n"
					)
					...
					# Append a row to the table (for later tabulation/logging)
					table.append([
						str(fp_dec),
						str(neuron_bw),
						str(w_bw),
					...
					# Collect into results list (returned to caller)
					results.append({
						"fp_dec": fp_dec,
						"weights_bw": w_bw,
						"neurons_bw": neuron_bw,
						"loss": float(loss),
						"accuracy": acc_pct
					})

		# After loops finish, render the full table and log it
		full_table = "\n" + tabulate(table, headers=headers, tablefmt="grid")
		logging.info(full_table)

		# Return the list of all (fp_dec, weights_bw, neurons_bw, loss, accuracy) dicts
		return results
